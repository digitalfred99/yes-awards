...
export const awardsApi = {
  ...
  users: (filters = "") => request<UserList>(`/users${filters ? `?${filters}` : ""}`).then(data => { const current = session.get(); return current?.user.role === "ADMIN" ? { ...data, users: data.users.filter(user => user.role === "NOMINEE") } : data; }),
  ...
  updateUser: (id: string, data: Partial<Pick<User, "firstName" | "lastName" | "nickName" | "gender" | "category" | "interest" | "nomineeCode">> & { password?: string }) => request<User>(`/users/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
