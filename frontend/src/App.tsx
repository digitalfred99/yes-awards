...
function CategoryManagementContent() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingId)
        await awardsApi.updateCategory(editingId, {
          name: name.trim(),
  ...
  const remove = async (category: Category) => {
    ...
    setSuccess("");
    try {
      await awardsApi.deleteCategories([category.id]);
  ...
  return (
    <section className="panel category-management">
      <div className="panel-title">
        <div>
          <p className="eyebrow">CATEGORY NAVIGATION</p>
          <h2>Manage categories</h2>
        </div>
        <Plus className="gold-icon" />
      </div>
      {error && <Notice text={error} />}{" "}
      {success && <Notice text={success} kind="success" />}
      <form className="category-form" onSubmit={submit}>
        <div className="category-fields">
          <Field label="Category name" value={name} onChange={setName} />
          <label>
            Description
            <textarea
              value={description}
              ...
              rows={2}
              placeholder="What does this category celebrate?"
            />
          </label>
        </div>
        <div className="category-form-actions">
          <button className="button" disabled={!name.trim() || saving}>
            {saving
              ? "Saving…"
              : editingId
                ? "Update category"
      ...
      {visible.length ? (
        <div className="managed-category-list">
          {visible.map((category) => (
            <article className="managed-category" key={category.id}>
              <div>
                <h3>{category.name}</h3>
        ...
        <Empty label="No categories match your search." />
      )}
    </section>
  );
}

function AdminResetImageCount({
  user,
  onUpdated,
}: {
  user: User;
  ...
  const reset = async () => {
    if (
      !confirm(`Reset ${user.firstName} ${user.lastName}'s image change count?`)
    )
      return;
    setSaving(true);
    setError("");
    try {
      onUpdated(await awardsApi.resetProfileChangeCount(user.id));
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "Unable to reset image changes.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="reset-count-control">
      {error && <Notice text={error} />}
      <button
        type="button"
        className="button mini reset-count-button"
        disabled={saving || user.profileChangeCount === 0}
        onClick={() => void reset()}
      >
        {saving ? "Resetting…" : "Reset image change count"}
      </button>
    </div>
  );
}
...
const formatName = (value: string) =>
  value
    ...
    .join(" ");
const uploadProfilePhotoWithProgress = async (
  id: string,
  file: File,
...
function useHash() {
  ...
  useEffect(() => {
    const hideNotice = (notice: Element) => {
...
function Notice({
  ...
  return (
    <div
      className={`notice${kind === "success" ? " success" : ""}`}
...
function Home() {
  ...
  useEffect(() => {
    awardsApi
      .categories()
      .then((x) => setCategories(x.categories.slice(0, 3)))
      .catch(() => {});
  }, []);
  return (
    <>
      ...
      <main>
        <section className="hero">
          ...
          <div className="hero-copy">
            <p className="eyebrow">ODOBEN'S NIGHT OF DISTINCTION</p>
            <h1>
              Own your <i>moment.</i>
            </h1>
            <p>
              {BRAND.slogan} Step forward, share your story and let the
              community celebrate what you bring to Odoben.
            </p>
            <div className="actions">
              <a className="button" href="#/register">
                Become a nominee <ArrowRight size={17} />
              </a>
        ...
        <section className="section" id="how-it-works">
          ...
          <h2>Every remarkable story deserves its stage.</h2>
          <div className="steps">
            <article>
              <span>01</span>
              <h3>Choose your category</h3>
              <p>
                Find the space that best celebrates your talent, service or
                impact.
              </p>
            </article>
            <article>
              <span>02</span>
              <h3>Register your story</h3>
              <p>Create your nominee profile in a few thoughtful steps.</p>
            </article>
            <article>
              <span>03</span>
              <h3>Join the celebration</h3>
              <p>Stay active and be part of the awards experience.</p>
            </article>
          </div>
        </section>
        <section className="section highlight">
          <div>
            ...
            <h2>Made for the people moving our community forward.</h2>
            <a className="link gold" href="#/categories">
              See every category <ArrowRight size={16} />
            </a>
          </div>
          <div className="category-list">
            {categories.map((c, i) => (
              <article key={c.id}>
                ...
                <p>
                  {c.description ||
                    "A category celebrating excellence in our community."}
                </p>
              </article>
            ))}
          </div>
        ...
        <section className="cta">
          <Crown />
          <h2>Your name belongs in the room.</h2>
          <a className="button" href="#/register">
            Start your nomination <ArrowRight size={17} />
          </a>
        </section>
      </main>
      <Footer />
...
function Categories() {
  ...
  return (
    <>
      <Header />
      <main className="page">
        <p className="eyebrow">FIND YOUR STAGE</p>
        <h1>A category for every kind of brilliance.</h1>
        ...
        {state === "loading" ? (
          <Loading label="Finding categories" />
        ) : state === "error" ? (
          <ErrorState message="We couldn't load the categories." retry={load} />
        ) : items.length ? (
          <div className="category-grid">
            {items.map((c, i) => (
              <article key={c.id} className="category">
                <span>0{i + 1}</span>
                <h2>{c.name}</h2>
                <p>
                  {c.description ||
                    "Celebrating outstanding contribution, drive and excellence."}
                </p>
                <a href="#/register">
...
function Register() {
  ...
  const [understandsVoting, setUnderstandsVoting] = useState(false);
  ...
  useEffect(() => {
    awardsApi
      ...
      .catch(() =>
        setError("Categories are unavailable. Please try again shortly."),
      );
  }, []);
  const valid =
    step === 1
      ...
      : step === 2
        ? form.category
        : Boolean(form.interest && understandsVoting);
  const submit = async () => {
    if (!valid) return;
    setSaving(true);
    setError("");
    try {
      const user = await awardsApi.createUser({
        ...form,
        firstName: formatName(form.firstName),
        lastName: formatName(form.lastName),
        nickName: form.nickName ? formatName(form.nickName) : "",
      });
      setCreated(user);
      setStep(4);
    } catch (e) {
  ...
  const change = <K extends keyof typeof form>(k: K, v: (typeof form)[K]) =>
    setForm((x) => ({
      ...x,
      [k]:
        k === "firstName" || k === "lastName" || k === "nickName"
          ? formatName(String(v))
          : v,
    }));
  if (created)
      ...
      <>
        ...
        <main className="form-page">
          <div className="success-panel">
            ...
            <h1>Welcome, {created.firstName}.</h1>
            <p>
              Your nominee account is ready. Add a flyer-ready photo now, or
              return whenever you are ready.
            </p>
            <a className="button" href="#/dashboard">
              Open my profile <ArrowRight size={17} />
            </a>
          </div>
        </main>
        <Footer />
    ...
    <>
      ...
      <main className="form-page">
        <div className="form-intro">
          <p className="eyebrow">BECOME A NOMINEE</p>
          <h1>This is where your story begins.</h1>
          <p>Complete your profile with care. It only takes a few minutes.</p>
        </div>
        <div className="progress">
          {["Your details", "Your category", "Your commitment"].map((x, i) => (
            <div key={x} className={step >= i + 1 ? "active" : ""}>
              <span>{i + 1}</span>
              {x}
            </div>
          ))}
        ...
        <div className="form-card">
            ...
            <>
              ...
              <div className="form-grid">
                ...
                <Field
                  ...
                  hint="This will be shown as your nominee name if you set one"
                  value={form.nickName ?? ""}
                  onChange={(v) => change("nickName", v)}
                />
                <Field
                  label="Whatsapp number"
                  hint="10 digits, e.g. 0240000000"
                  type="tel"
                  inputMode="numeric"
                  maxLength={10}
                  value={form.phoneNumber}
                  onChange={(v) =>
                    change("phoneNumber", v.replace(/\D/g, "").slice(0, 10))
                  }
                />
                <label>
                  Gender
          ...
          {step === 2 && (
            <>
              ...
              <p className="muted">
                Search and select the one category that best represents you.
              </p>
              <CategoryPicker
          ...
          {step === 3 && (
            <>
              ...
              <p className="muted">
                Tell us how you plan to show up for the awards experience.
              </p>
              <CommitmentPicker
                value={form.interest ?? ""}
                onChange={(value) => change("interest", value || undefined)}
              />
              <div className="nominee-agreement">
                <p className="agreement-label">Nominee Agreement</p>
                <label className="consent-checkbox">
                  <input
                    type="checkbox"
                    checked={understandsVoting}
                    onChange={(e) => setUnderstandsVoting(e.target.checked)}
                  />
                  <span>
                    I understand that becoming a winner requires a minimum
                    number of votes. Each vote costs GH₵0.50, and I agree to
                    participate in the voting process to compete for the
                    crown🏆
                  </span>
                </label>
              </div>
    ...
    <div className="category-picker" ref={pickerRef}>
            ...
            {visible.length ? (
              visible.map((c) => (
                <button
                  ...
                  className={value === c.name ? "selected" : ""}
                  onClick={() => {
                    onChange(c.name);
                    setOpen(false);
                    setQuery("");
                ...
                >
                  ...
                  <span>{value === c.name && <Check size={15} />}</span>
                </button>
              ))
            ) : (
      ...
      {value && (
        <p className="small-note">
          Selected category: <strong>{value}</strong>
        </p>
      )}
    </div>
  );
...
function CommitmentPicker({
  ...
  const [open, setOpen] = useState(false);
  const pickerRef = useRef<HTMLDivElement>(null);
  const options: Array<{ value: Interest; label: string }> = [
    { value: "EXPLORING", label: "Exploring the opportunity" },
    { value: "COMMITTED", label: "Committed to participate" },
    { value: "HIGHLY_COMMITTED", label: "Highly committed" },
    { value: "FULLY_COMMITTED", label: "Fully committed" },
  ];
  useEffect(() => {
    const closeOnOutsideClick = (event: MouseEvent) => {
      if (!pickerRef.current?.contains(event.target as Node)) setOpen(false);
  ...
  return (
    <div className="category-picker" ref={pickerRef}>
      ...
      {selected && (
        <p className="small-note">
          Selected commitment: <strong>{selected.label}</strong>
        </p>
      )}
    </div>
  );
...
function Field({
  ...
  return (
    <label>
      {label}
      <span className={secret ? "input-with-action" : undefined}>
        <input
...
function Login() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const data = await awardsApi.login(phone, password);
      session.set(data);
      location.hash = data.user.role === "NOMINEE" ? "/dashboard" : "/admin";
    } catch (err) {
    ...
    <>
      ...
      <main className="auth">
        <form className="auth-card" onSubmit={submit}>
          ...
          <p className="muted">
            New here? <a href="#/register">Become a nominee</a>
          </p>
        </form>
      </main>
      <Footer />
    ...
    <>
      ...
      <main className="auth">
        <div className="auth-card">
          <Heart className="big-icon" />
          <p className="eyebrow">WE'RE HERE TO HELP</p>
          <h1>Reset your password.</h1>
          <p>
            Our award administrators will securely help you regain access over
            WhatsApp.
          </p>
          <button className="button" onClick={() => openResetWhatsApp()}>
            Message an administrator <ArrowRight size={17} />
          </button>
        </div>
      </main>
      <Footer />
...
function Dashboard() {
  ...
  const save = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const fresh = await awardsApi.updateUser(user.id, {
  ...
  const pick = async (file: File) => {
    ...
    try {
      setUser(
        await uploadProfilePhotoWithProgress(user.id, file, setPhotoProgress),
      );
    } catch (e) {
      ...
      <section className="dashboard-grid">
        <div className="profile-main">
          ...
          <div className="panel">
            <div className="panel-title">
              ...
              <button className="text-btn" onClick={() => setEdit(!edit)}>
            ...
            {edit ? (
              ...
              <dl>
                ...
                <div>
                  <dt>Participation</dt>
                  <dd>
                    <span className={user.isActive ? "pill green" : "pill"}>
        ...
        <aside className="side-stack">
          <div className="panel">
            ...
            <h3>Stay active</h3>
            <p>
              Your active status shows your continued interest in the programme.
            ...
            <button
              ...
              onClick={() =>
                confirm(
                  "Changing activity status is not supported by the current API. Inactivity may be considered a loss of interest and could result in removal from programme activities and the WhatsApp group.",
                )
              }
            >
              About inactivity
          ...
          <div className="panel">
            ...
            <p>
              Request a password reset directly from an awards administrator.
            </p>
            <button
              className="button mini"
              onClick={() =>
                openResetWhatsApp(`${user.firstName} ${user.lastName}`)
              }
            >
              Contact on WhatsApp
            </button>
...
function AdminUserDetailsContent({
  ...
  const upload = async (file: File) => {
    ...
    setError("");
    try {
      onUpdated(await awardsApi.uploadPhoto(user.id, file));
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "Unable to update profile image.",
      );
    } finally {
      setUploading(false);
    }
  ...
  return (
    <section className="panel admin-user-details">
      <div className="panel-title">
        <div>
      ...
      {isNominee && (
        <PhotoUpload
          ...
          onLimitHelp={() =>
            setError(
              "This user has used all profile image changes allowed by the programme.",
            )
          }
        />
      )}
      <dl>
        ...
        {isNominee && user.nomineeCode && (
          <div>
            <dt>Nominee code</dt>
            <dd>{user.nomineeCode}</dd>
          </div>
        )}
        {isNominee && (
          <div>
            ...
            <dd>
              {user.interest
                ? user.interest.replace(/_/g, " ").toLowerCase()
                : "—"}
            </dd>
          </div>
        )}
...
function AdminSelfProfile() {
  ...
  return (
    <section className="dashboard-grid admin-self-profile">
      <div className="profile-main">
...
function AdminEditProfile({
  ...
  const [nomineeCodeValue, setNomineeCodeValue] = useState(
    user.nomineeCode ?? "",
  );
  const isNominee = user.role === "NOMINEE";
  ...
  const save = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      onUpdated(
        await awardsApi.updateUser(user.id, {
          ...
          ...(isNominee
            ? {
                ...
                nomineeCode: nomineeCodeValue.trim() || null,
              }
            : {}),
        }),
      );
    ...
    <form className="form-grid" onSubmit={save}>
      ...
      {isNominee && (
        <label>
          Nominee code
          <input
            value={nomineeCodeValue}
            onChange={(e) => setNomineeCodeValue(e.target.value)}
            placeholder="e.g. OY-2026-014"
          />
        </label>
      )}
      <div className="form-actions">
...
function AdminCreateUser({
  ...
  const [error, setError] = useState("");
  const [success, setSuccess] = useState("");
  const change = <K extends keyof CreateUser>(key: K, value: CreateUser[K]) =>
    setForm((current) => ({
      ...current,
      [key]:
  ...
  const valid =
    ...
    /^\d{10}$/.test(form.phoneNumber) &&
    form.password.length >= 8 &&
    (form.role !== "NOMINEE" || form.category);
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!valid) return;
    setSaving(true);
    ...
    setSuccess("");
    try {
      const normalizedFirstName = formatName(form.firstName);
      const normalizedLastName = formatName(form.lastName);
      const normalizedNickName = form.nickName ? formatName(form.nickName) : "";
      const payload =
        form.role === "NOMINEE"
          ? {
              ...form,
      ...
      await awardsApi.createUser(payload);
      setForm({
        firstName: "",
        lastName: "",
        nickName: "",
    ...
    <section className="panel admin-form">
      ...
      <form className="form-grid" onSubmit={submit}>
        ...
        <Field
          ...
          hint="10 digits, e.g. 0240000000"
          value={form.phoneNumber}
          onChange={(value) => change("phoneNumber", value)}
        />
        <label>
          Gender
          <select
            value={form.gender}
            onChange={(event) => change("gender", event.target.value as Gender)}
          >
            <option value="MALE">Male</option>
            <option value="FEMALE">Female</option>
          </select>
        ...
        <label>
          ...
          <select
            value={form.role}
            onChange={(event) =>
              change("role", event.target.value as CreateUser["role"])
            }
          >
          ...
          <>
            ...
            <label>
              ...
              <select
                value={form.interest}
                onChange={(event) =>
                  change("interest", event.target.value as Interest)
                }
              >
                <option value="EXPLORING">Exploring</option>
                <option value="COMMITTED">Committed</option>
                <option value="HIGHLY_COMMITTED">Highly committed</option>
                <option value="FULLY_COMMITTED">Fully committed</option>
              </select>
            </label>
          </>
        )}
        <button className="button" disabled={!valid || saving}>
...
function ResetUserPassword({
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setError("");
    try {
      await awardsApi.updateUser(user.id, { password });
      onDone();
    } catch (e) {
    ...
    <section className="panel reset-panel">
      ...
      <form className="form-actions" onSubmit={submit}>
        <Field
          ...
          onChange={setPassword}
        />
        <button className="button" disabled={password.length < 8 || saving}>
...
function AdminDirectory() {
  ...
  const canCreateAdmins = active?.user.role === "SUPER_ADMIN";
  const load = () => {
  ...
  useEffect(load, [roleFilter, gender, status]);
  useEffect(() => {
    awardsApi
      .categories()
      .then((x) => setCategories(x.categories))
  ...
  const filtered = useMemo(
    () =>
      users.filter((user) =>
        `${user.firstName} ${user.lastName} ${user.phoneNumber} ${user.nickName || ""} ${user.category} ${user.role}`
          .toLowerCase()
          .includes(query.toLowerCase()),
      ),
    [users, query],
  );
  const canReset = (user: User) =>
  ...
  const deleteUser = async (user: User) => {
    ...
    setSuccess("");
    try {
      await awardsApi.deleteUsers([user.id]);
  ...
  if (!active || active.user.role === "NOMINEE") {
    location.hash = "/login";
    return null;
  }
  return (
    <main className="dash">
      <DashboardHeader title="Awards administration" />
      {error && <Notice text={error} />}{" "}
      {success && (
      ...
      <div className="stat-row">
        <Stat icon={<UsersRound />} label="Users" value={users.length} />
        <Stat
          icon={<ShieldCheck />}
          label="Active"
        ...
        <Stat
          icon={<Crown />}
          label="Your access"
          value={prettyRole(active.user.role)}
        />
      </div>
      <AdminCreateUser
      ...
      <section className="panel">
        ...
        <div className="filters">
          <div className="search">
            <Search size={17} />
            <input
              placeholder="Search name, phone, nickname or category"
              value={query}
              onChange={(event) => setQuery(event.target.value)}
            />
          </div>
                ...
                {filtered.map((user) => (
                  <tr key={user.id} data-user-id={user.id}>
                    <td>
                      ...
                      <small>{user.nickName || "No nickname"}</small>
                    </td>
                    <td>{prettyRole(user.role)}</td>
                    <td>{user.category || "—"}</td>
                    <td>{user.nomineeCode || "—"}</td>
...
function DashboardEnhanced() {
  ...
  const toggleStatus = async () => {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : "Unable to update your status.",
      );
    } finally {
      ...
      <section className="dashboard-grid">
        <div className="profile-main">
          <PhotoUpload
            ...
            onPick={pick}
            onLimitHelp={() =>
              openPhotoChangeWhatsApp(`${user.firstName} ${user.lastName}`)
            }
          />
          <div className="panel">
            <p className="eyebrow">YOUR AWARDS PROFILE</p>
            <h2>{user.category}</h2>
            <p>
              Your commitment: {user.interest.replace(/_/g, " ").toLowerCase()}.
            </p>
            <dl>
              <div>
                <dt>Full name</dt>
        ...
        <aside className="side-stack">
          <div className="panel">
            ...
            <p>
              You are currently{" "}
              <span
                className={user.status === "ACTIVE" ? "pill green" : "pill"}
              >
...
function PersonalInfo({
  ...
  const saveCategory = async () => {
    if (!categoryValue || categoryValue === user.category) {
    ...
    try {
      onUpdated(
        await awardsApi.updateUser(user.id, { category: categoryValue }),
      );
      setEditingCategory(false);
    } catch (e) {
      setCategoryError(
        e instanceof Error ? e.message : "Unable to update your category.",
      );
    } finally {
      ...
      {editing ? (
        <form className="form-grid" onSubmit={save}>
          ...
          {isNominee && user.nomineeCode && (
            <label>
              Nominee code
              <input value={user.nomineeCode} disabled />
            </label>
          )}
          <button className="button" disabled={saving}>
            {saving ? "Saving…" : "Save changes"}
        ...
        <dl>
            ...
            <div>
              ...
              >
                ...
                {!editingCategory && (
                  <button
                    ...
                    className="text-btn"
                    onClick={() => {
                      setCategoryValue(user.category);
                      setCategoryError("");
              ...
              {editingCategory && (
                <div style={{ marginTop: 12 }}>
                  {categoryError && <Notice text={categoryError} />}
                  <CategoryPicker
                    categories={categories}
                    value={categoryValue}
                    onChange={setCategoryValue}
          ...
          {isNominee && (
            <div>
              <dt>Commitment</dt>
              <dd>{user.interest.replace(/_/g, " ").toLowerCase()}</dd>
            </div>
          )}
        </dl>
      )}
...
function AdminDetailsRoute() {
  ...
  useEffect(() => {
    ...
    awardsApi
      ...
      .catch((e) =>
        setError(
          e instanceof Error ? e.message : "Unable to load user details.",
        ),
      );
  }, [selected]);
    ...
    >
      ...
      >
        ...
        {error ? (
          ...
          <AdminUserDetails user={user} onUpdated={setUser} onClose={close} />
        )}
      </section>
    </div>
  );
...
function AdminUserDetails({
  ...
  const canEdit =
    isSuperAdmin ||
    isSelf ||
    (active?.user.role === "ADMIN" && user.role === "NOMINEE");
  return (
    <section className="admin-details-stack">
      ...
      />
      {canEdit && (
        <div className="admin-count-action">
          <AdminEditProfile user={user} onUpdated={onUpdated} />
        </div>
      )}
      {user.role === "NOMINEE" && (
        <div className="admin-count-action">
          <AdminResetImageCount user={user} onUpdated={onUpdated} />
        </div>
      )}
    </section>
  );
...
function AdminShell({ modal = false }: { modal?: boolean }) {
  useEffect(() => {
    const openDetails = (event: MouseEvent) => {
      ...
      if (id) location.hash = `/admin?user=${encodeURIComponent(id)}`;
    };
    document.addEventListener("click", openDetails);
    return () => document.removeEventListener("click", openDetails);
  }, []);
  return (
    <div className={`admin-shell-content${modal ? " admin-shell-modal" : ""}`}>
      <AdminDirectory />
      <AdminDetailsRoute />
    </div>
  );
    ...
    >
      ...
      >
        <div className="modal-heading">
          <div>
            <p className="eyebrow">ACCOUNT MANAGEMENT</p>
            <h2 id="account-management-title">Manage accounts</h2>
          </div>
          <button
            type="button"
            className="icon-button"
  ...
  return (
    <div className="admin-page">
      ...
      <div className="admin-management-nav">
        ...
        >
          <UsersRound size={17} /> Manage accounts
        </button>
        <button
          type="button"
          className="button"
      ...
      {accountModalOpen && (
        <AccountManagementModal onClose={() => setAccountModalOpen(false)} />
      )}{" "}
      {categoryModalOpen && (
        <CategoryManagementModal onClose={() => setCategoryModalOpen(false)} />
      )}
    </div>
  );
}
export default function App() {
  ...
  if (path === "/dashboard") return <DashboardComplete />;
