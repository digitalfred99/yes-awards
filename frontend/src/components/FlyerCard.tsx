...
export function FlyerCard({
  ...
  if (!ENABLE_FLYER_GENERATION) {
    return (
      <div className="panel flyer-card">
        <p>Flyer generation is temporarily disabled.</p>
      </div>
    );
  }
  ...
  const renderBlob = async (): Promise<Blob> => {
    if (!hasProfileImage) {
      throw new Error("Please add a profile photo before generating the flyer.");
    }
  ...
  const download = async () => {
    if (!hasProfileImage) {
      setError("Please add a profile photo before generating the flyer.");
      setStatus("error");
      return;
    }
      ...
      {!isGenerated ? (
        <div className="flyer-empty-state">
          <button
            type="button"
            className="button"
            disabled={isGenerating || !hasProfileImage}
            onClick={() => void generateFlyer()}
          >
            {isGenerating
              ? "Generating..."
              : !hasProfileImage
                ? "Add photo to generate flyer"
                : "Generate Nominee Flyer"}
          </button>
        </div>
      ) : (
        <>
