...
export function PhotoUpload({ user, pending, isAdmin, onPick, onLimitHelp }: { user: Pick<User, "profileImage" | "profileChangeCount" | "firstName">; pending?: boolean; isAdmin?: boolean; onPick: (file: File) => void; onLimitHelp?: () => void }) {
  ...
  return <section className="photo-card">
    ...
    <div>
      ...
      {disabled ? <button className="button muted-button" onClick={onLimitHelp}><LockKeyhole size={16}/> Contact an admin to change it</button> : <label className="button mini"><Camera size={16}/>{uploadLabel}<input hidden type="file" accept="image/*" onChange={(e) => e.target.files?.[0] && onPick(e.target.files[0])}/></label>}
